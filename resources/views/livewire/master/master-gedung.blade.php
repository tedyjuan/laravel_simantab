<div id="halaman_master_tahun_ajar" x-data="{ modalOpen: false, deleteModalOpen: false, modalTitle: '', modalTitleDetail: '' }" x-on:open-modal.window="modalOpen = true"
    ...
    <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button type="button"
            @click="
                modalOpen = true;
                modalTitle = 'Tambah Gedung';
                modalTitleDetail = 'Lengkapi data Gedung baru';
                $wire.set('gedung_id', null, false);
                $wire.set('kode_gedung', '', false);
                $wire.set('nama_gedung', '', false);
                $wire.set('alamat', '', false); 
                $wire.set('jumlah_lantai', '', false);
                            ...
                            <td class="pr-6">
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    <button type="button" wire:click="edit({{ $item->id }})" title="Edit"
                                        @click="
                                            modalTitle = 'Edit Gedung';
                                            modalTitleDetail = 'Perbarui data Gedung';
                                        "
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-14 text-center">
                                <span class="icon-[carbon--building] mx-auto mb-2 block size-8 text-[#D8D5EC]"></span>
    ...
    <div x-show="modalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="relative z-10 max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <form wire:submit="store" class="space-y-4">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1 block text-xs font-medium text-[#544F7A]">Jumlah Lantai</label>
                        <input type="number" wire:model="jumlah_lantai"
                            class="input w-full border-[#ECE9F7] bg-[#FAFAFD] text-sm" />
                        @error('jumlah_lantai')
                            <span class="mt-1 block text-xs text-red-500">{{ $message }}</span>
                        @enderror
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-1">
                    <div>
                        <label class="mb-1 block text-xs font-medium text-[#544F7A]">Alamat</label>
                        <textarea wire:model="alamat" class="textarea  w-full border-[#ECE9F7] bg-[#FAFAFD] text-sm" aria-label="Textarea"></textarea>
                        @error('alamat')
                            <span class="mt-1 block text-xs text-red-500">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="mt-6 flex items-center justify-end gap-2">
                    ...
                    <button type="submit" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove wire:target="store">
                            {{ $gedung_id ? 'Simpan Perubahan' : 'Simpan Data' }}
                        </span>
